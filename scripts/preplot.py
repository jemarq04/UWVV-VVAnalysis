...
def main():
    ...
    with ROOT.TFile.Open(args.infile, "update") as infile:
        if any(name in [key.GetName() for key in infile.GetListOfKeys()] for name in ["analysis", "year"]):
            parser.error("file already processed with preplot!")

        for name in ["analysis", "year"]:
            infile.WriteObject(ROOT.TNamed(name, getattr(args, name)), name)
        ...
        for sample in mergetools.get_children(infile, ROOT.TDirectory):
            ...
            # Any analysis-specific steps per sample
            mergetools.preplot_sample(sample, args.analysis)
        ...
        for combined_group, plot_groups in groups["combined"].items():
            ...
            if combined_group == "AllData":
                ...
                members = [
                    sample_name for group_name in plot_groups for sample_name in groups["groups"][group_name]["members"]
                ]
            ...
            if summed_hists:
                combined_dir = infile.mkdir(combined_group)
        ...
        if all(infile.FindObject(name) for name in ["AllData", "AllEWK"]):
            if args.verbose:
                print("Correcting data with EWK MC...")
            differences = {}
            for subdir in mergetools.get_children(infile.Get("AllData"), ROOT.TDirectory):
                subdir_name = subdir.GetName()
                differences[subdir_name] = ROOT.TList()
                for hist in mergetools.get_children(subdir, ROOT.TH1):
                    differences[subdir_name].Add(hist.Clone())
            for subdir in mergetools.get_children(infile.Get("AllEWK"), ROOT.TDirectory):
                subdir_name = subdir.GetName()
                for hist in mergetools.get_children(subdir, ROOT.TH1):
                    differences[subdir_name].FindObject(hist.GetName()).Add(hist, -1)

            differences_dir = infile.mkdir("DataEWKCorrected")
